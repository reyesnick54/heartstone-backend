...
export class EvidenceRecordsService {
  ...
  async receiveEvidence(_submitterIdentityId: string, dto: ReceiveEvidenceDto) {
    ...
    const masterAdministrativeFileId = caseRecord.masterAdministrativeFile?.id;
    if (!masterAdministrativeFileId) {
      throw new BadRequestException(
        'Evidence cannot be registered until a master administrative file exists for the case',
      );
    }
