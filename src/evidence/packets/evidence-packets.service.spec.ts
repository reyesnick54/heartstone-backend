...
describe('EvidencePacketsService', () => {
  ...
  const prisma = {
    evidencePacket: { count: jest.fn(), create: jest.fn(), findUnique: jest.fn() },
    ...
    evidencePacketItemExclusion: { create: jest.fn() },
    evidencePacketManifest: { create: jest.fn() },
    evidenceRecord: { findUnique: jest.fn(), findMany: jest.fn() },
  ...
  it('allows draft packet item mutation', async () => {
    ...
    prisma.documentVersion.findUnique.mockResolvedValue({ id: 'doc-1', sha256: 'hash-1' });
  ...
  it('pins exact document and evidence versions on inclusion', async () => {
    ...
    prisma.documentVersion.findUnique.mockResolvedValue({ id: 'doc-v3', sha256: 'hash-v3' });
  ...
  it('detects unresolved government objections as must-surface evidence', async () => {
    prisma.evidencePacketVersion.findFirst.mockResolvedValue({
      ...draftVersion,
      packet: { masterAdministrativeFileId: 'maf-1' },
      items: [],
    ...
    prisma.evidenceRecord.findMany.mockResolvedValue([
      {
        id: 'gov-1',
        evidenceNumber: 'ER-GOV',
        status: EvidenceRecordStatus.RECEIVED,
        governmentCommunicationEvidence: [
          { governmentCommunication: { category: GovernmentCommunicationCategory.OBJECTION } },
        ],
        professionalReviewEvidence: [],
      },
    ]);
  ...
  it('stores manifest hash that changes when packet content changes', () => {
      ...
      items: [
        {
          itemId: 'i1',
          evidenceRecordId: 'r1',
          evidenceStatusAtInclusion: 'RECEIVED',
    ...
    const firstItem = manifestA.items[0];
    if (!firstItem) {
      throw new Error('expected manifest item');
    }
    const manifestB = {
      ...manifestA,
      items: [{ ...firstItem, documentContentHash: 'hash-b' }],
    };
