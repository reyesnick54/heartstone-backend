...
describe('Healthcare must-fail gates', () => {
  ...
  describe('Healthcare consent and access policy', () => {
    ...
    let accessPolicy: HealthcareDataAccessPolicyService;
    let prisma: {
    ...
    beforeEach(async () => {
      ...
      accessPolicy = module.get(HealthcareDataAccessPolicyService);
    });
    ...
    it('health data access is audited when denied for platform admin', async () => {
      ...
      await expect(
        accessPolicy.assertMayAccessHealthcareData(buildActor(), {
          patientReferenceId: 'patient-1',
          purposeCode: 'TREATMENT',
          classification: HealthcareDataClassification.GENETIC,
          endpoint: 'test',
          roleMarker: PLATFORM_ADMIN_HEALTHCARE_ROLE_MARKER,
        }),
