  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InstrumentLifecycleDecisionService, useValue: decisionService },
      ],
    }).compile();
  ...
  describe('boundary guards', () => {
    it('rejects ordinary PATCH of instrument status', () => {
      expect(() => { boundary.assertClientCannotPatchInstrumentStatus({
          lifecycleStatus: LifecycleInstrumentStatus.REVOKED,
        }); },
  ...
  describe('appeal and stay', () => {
    it('files review without auto-stay', async () => {
      ...
      const review = await service.fileReviewReference({
        challengedLifecycleDecisionId: 'decision-1',
  ...
  describe('amendment preserves original version', () => {
    it('creates new version and supersedes prior without deleting', async () => {
      decisionService.assertDecisionFinalized.mockResolvedValue({
        ...
        lifecycleDecisionType: InstrumentLifecycleDecisionType.AMEND,
      });

      const activeInstrument = {
        id: 'inst-1',
        lifecycleStatus: LifecycleInstrumentStatus.EFFECTIVE,
        currentVersion: {
          id: 'v1',
          versionNumber: 1,
      ...
      const amendedInstrument = {
        id: 'inst-1',
        lifecycleStatus: LifecycleInstrumentStatus.AMENDED,
      ...
      expect(result.lifecycleStatus).toBe(LifecycleInstrumentStatus.AMENDED);
    });
  });
    ...
    it('updates verification cache to non-current status after suspension', async () => {
      ...
      expect(prisma.lifecycleOfficialInstrument.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            publicVerificationStatus: 'SUSPENDED',
            lifecycleStatus: LifecycleInstrumentStatus.SUSPENDED,
          }) as Record<string, unknown>,
        }),
      );
    });
