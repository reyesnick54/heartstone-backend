...
import { MASTER_FILE_NUMBER_PREFIX, MASTER_FILE_SECTION_DEFINITIONS } from './records.constants';
...
export class MasterAdministrativeFileService {
  ...
  async initializeForCase(
    input: InitializeMasterFileInput,
  ): Promise<MasterAdministrativeFileWithSections> {
    ...
    const workflowVersionId = caseRecord.workflowVersionId;
    ...
    for (const office of offices) {
      ...
      if (office.departmentId !== input.responsibleDepartmentId) {
        throw new ConflictException(
          'Master file offices must belong to the responsible department',
        );
      }

      if (office.department.institutionId !== input.responsibleInstitutionId) {
        throw new ConflictException(
          'Master file offices must belong to the responsible institution',
        );
      }
    }
  }
