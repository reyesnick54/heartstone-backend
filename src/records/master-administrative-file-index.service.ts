...
export class MasterAdministrativeFileIndexService {
  ...
  async buildIndex(
    ...
    const [
      ...
      this.prisma.caseWorkflowInstance.findMany({
        where: { caseId: file.caseId },
        orderBy: { createdAt: 'asc' },
      }),
  ...
  private buildSectionReferences(
    ...
    context: {
      ...
      workflowInstances: Awaited<ReturnType<PrismaService['caseWorkflowInstance']['findMany']>>;
      milestones: Awaited<ReturnType<PrismaService['caseMilestone']['findMany']>>;
    },
    ...
    switch (sectionType) {
      case MasterAdministrativeFileSectionType.FILE_CONTROL_AND_INDEX:
        return [
          ...
          },
          {
            ...
            metadata: {
              caseStatus: context.caseRecord?.caseStatus,
        ...
        return context.application
          ? [
              ...
              },
              {
                ...
                label: context.application.applicationNumber,
              },
            ]
          : [];
      ...
      case MasterAdministrativeFileSectionType.AUTHORITY_AND_PROCEDURE:
        return context.workflowInstances.map((instance) => ({
          referenceType: 'CaseWorkflowInstance',
          referenceId: instance.id,
          label: instance.workflowVersionReference,
          metadata: {
      ...
      case MasterAdministrativeFileSectionType.COMPLETENESS_REVIEW:
        return context.events
          .filter((event) =>
            ['COMPLETENESS_STARTED', 'COMPLETENESS_COMPLETED', 'DEFICIENCY_ISSUED'].includes(
              event.eventType,
            ),
          )
          .map((event) => ({
      ...
      case MasterAdministrativeFileSectionType.GOVERNMENT_REFERRALS:
        return context.events
          .filter((event) =>
            ['REFERRED', 'REFERRAL_ACKNOWLEDGED', 'REFERRAL_RESPONSE_RECEIVED'].includes(
              event.eventType,
            ),
          )
          .map((event) => ({
      ...
      case MasterAdministrativeFileSectionType.COMMUNICATIONS_AND_NOTICES:
        return context.communications.map((communication) => ({
          ...
          label: communication.communicationType,
