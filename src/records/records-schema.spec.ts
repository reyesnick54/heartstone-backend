...
describe('Records schema coherence (Phase 7A)', () => {
  ...
  it('keeps one master file per case and immutable file number', () => {
    ...
    expect(block).toContain('fileNumber                           String                                    @unique');
    expect(block).toContain('caseId                               String                                    @unique');
    expect(block).toContain('administrativeOwnerOfficeId          String');
    expect(block).toContain('recordsCustodianOfficeId             String');
  });
});
