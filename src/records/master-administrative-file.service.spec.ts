...
describe('MasterAdministrativeFileService', () => {
  ...
  it('asserts file number immutability', async () => {
    ...
    await expect(service.assertFileNumberImmutable('file-1', 'MAF-2026-000002')).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('creates all twenty section types during initialization', async () => {
    ...
    prisma.office.findFirst
      .mockResolvedValueOnce({ id: 'office-admin', code: 'ADMIN', status: 'ACTIVE', department: { institutionId: 'inst-1' } })
      .mockResolvedValueOnce({ id: 'office-records', code: 'RECORDS', status: 'ACTIVE', department: { institutionId: 'inst-1' } });
    prisma.office.findMany.mockResolvedValue([
      { id: 'office-admin', code: 'ADMIN', status: 'ACTIVE', departmentId: 'dept-1', department: { institutionId: 'inst-1' } },
      { id: 'office-records', code: 'RECORDS', status: 'ACTIVE', departmentId: 'dept-1', department: { institutionId: 'inst-1' } },
    ]);
