...
describe('Redress boundary service (Phase 10H)', () => {
  ...
  it('3. AI assistance does not equal redress decision', () => {
    ...
    }).toThrow('AI assistance cannot record redress dispositions');
  });

  it('5. classification does not equal disposition', () => {
    expect(() => {
      boundary.assertClassificationNotDisposition(true, true);
    }).toThrow('Classification does not equal disposition');
  });
  ...
  it('8. clarification does not alter substantive decision', () => {
    ...
    }).toThrow('Clarification cannot alter substantive decision');
  });
  ...
  it('12. reconsideration preserves original decision record', () => {
    expect(() => {
      boundary.assertOriginalPreserved(false);
    }).toThrow('Original government decision must be preserved during redress');
  });
  ...
  it('19. original government decision preserved during redress', () => {
    expect(() => {
      boundary.assertOriginalPreserved(true);
    }).not.toThrow();
  });
  ...
  it('21-35. client cannot set protected redress fields', () => {
    for (const field of FORBIDDEN_CLIENT_REDIST_FIELDS) {
      expect(() => {
        boundary.rejectClientProtectedFields({ [field]: 'override' });
  ...
  it('boundary disclaimer is explicit about AI and disposition', () => {
    ...
    expect(PHASE_10H_BOUNDARY_DISCLAIMER).toMatch(
      /Filing, classification, and investigation do not establish/i,
    );
  });
});
